...
//CRUD-LIKE OPERATIONS
    ...
    private int searchLevel(Node node, String searchData, int level) {
        ...
        if(node.data.equalsIgnoreCase(searchData)){
...
//    }
    ...
    //TESTINGIN MO KUNG GUMAGANA YUNG BALANCED BINARY TREE
    
        // Helper function to create a balanced binary tree from a sorted array
    public Node balancedBinaryTreeHelper(String arr[], int start, int end) {
        if (start > end) {
            return null;
        }
        ...
        node.left = balancedBinaryTreeHelper(arr, start, mid - 1);
        node.right = balancedBinaryTreeHelper(arr, mid + 1, end);
